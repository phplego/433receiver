...
void mqtt_connect() {
    ...
    Serial.print("\nconnecting...");
    while (!mqttClient.connect("433receiver", MQTT_USER, MQTT_PASS)) {
        Serial.print(".");
        delay(1000);
    }
    ...
    mqttClient.subscribe("wifi2mqtt/433receiver/set");
}
...
void setup() 
{
  ...
  bool ok = mqttClient.publish("wifi2mqtt/433receiver", "started");
...
void loop() 
{
  if (mySwitch.available()) {
    ...
    mqttClient.publish("wifi2mqtt/433receiver", json);
